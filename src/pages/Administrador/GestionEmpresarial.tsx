...
type Objetivo  = ObjetivoAPI;
type AlertaItem = AlertaAPI;
...
function ModalObjetivo({ obj, onClose }: { obj: Objetivo; onClose: () => void }) {
  const diasRestantes = Math.ceil(
    (new Date(obj.fecha_limite).getTime() - Date.now()) / 86400000
  );
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md mx-4 rounded-2xl bg-white dark:bg-[#18110d] shadow-2xl">
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-200 dark:border-[#2b2b2b]">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full text-gray-500 hover:bg-gray-100 dark:text-[#c9a46b] dark:hover:bg-[#1a1a1a] transition-colors"
          >
            ✕
          </button>
        </div>
        <div className="px-6 py-5 space-y-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800 dark:text-[#e8c48e]">
              {obj.titulo}
            </h2>
            <p className="mt-1 text-sm text-gray-500 dark:text-[#8b6b42]">
              {obj.descripcion}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              >
                ...
                <p className="mt-0.5 text-sm font-semibold text-gray-800 dark:text-[#e8c48e]">
                  {item.value}
                </p>
              </div>
            ))}
          ...
          <div>
            <div className="flex justify-between text-xs mb-1">
              <span className="text-gray-500 dark:text-[#8b6b42]">
                Avance del objetivo
              </span>
              <span className="font-semibold text-gray-700 dark:text-[#c9a46b]">
                {obj.progreso}%
              </span>
            </div>
            <BarraProgreso
              pct={obj.progreso}
              color="bg-amber-500 dark:bg-[#d4a15d]"
            />
          </div>
        </div>
      </div>
...
export default function GestionEmpresarial() {
  ...
  const [metricas,     setMetricas]     = useState<MetricaAPI[]>([]);
  ...
  // ─── Métricas derivadas de objetivos ──────────────────
  ...
  const pctGlobal   =
    objetivos.length > 0
      ? Math.round(
          objetivos.reduce((a, o) => a + o.progreso, 0) / objetivos.length
        )
      : 0;
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-gray-800 dark:text-[#e8c48e]">
        Gestión Empresarial
      </h1>
      <p className="mt-2 text-gray-600 dark:text-[#c9a46b]">
        Resumen general de procesos estratégicos.
      </p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {metricas.map((m) => (
          <div
            key={m.id_metrica}
            className="rounded-2xl bg-white dark:bg-[#18110d] p-5 shadow-sm"
          >
            <p className="text-xs text-gray-400 dark:text-[#7a5c3a] uppercase tracking-wide">
              {m.label}
            </p>
            <p className={`mt-1 text-2xl font-bold ${m.text_color}`}>
              {m.valor}
            </p>
            <p className="text-xs text-gray-500 dark:text-[#8b6b42] mt-0.5">
              {m.sub}
            </p>
            {m.pct !== null && (
              <div className="mt-2">
                <BarraProgreso pct={Number(m.pct)} color={m.color} />
              </div>
            )}
          </div>
        ))}
      ...
      <div className="mt-6 rounded-2xl bg-white dark:bg-[#18110d] shadow-sm p-5">
        <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
          ...
          <span className="text-sm text-gray-500 dark:text-[#8b6b42]">
            Avance global:{" "}
            <strong className="text-gray-800 dark:text-[#e8c48e]">
              {pctGlobal}%
            </strong>
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {[
            ...
            <div key={s.label} className={`rounded-xl ${s.bg} p-3 text-center`}>
              ...
              <p className="text-xs text-gray-500 dark:text-[#8b6b42]">
                {s.label}
              </p>
            </div>
          ))}
      ...
      {/* ── Alertas ── */}
      <div className="mt-6 rounded-2xl bg-white dark:bg-[#18110d] shadow-sm p-5">
        <h2 className="text-lg font-bold text-gray-800 dark:text-[#e8c48e] mb-3">
          Alertas y Novedades
        </h2>
        <div className="space-y-3">
          {alertas.map((a) => {
            ...
            return (
              <div
                key={a.id_alerta}
                className="flex items-start gap-3 rounded-xl bg-gray-50 dark:bg-[#120c08] overflow-hidden"
              >
                ...
                <div className="py-3 pr-3 flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-700 dark:text-[#c9a46b]">
                    {a.mensaje}
                  </p>
                </div>
              </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-[#120c08]">
                {["Objetivo", "Área", "Responsable", "Avance", "Estado", "Límite"].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 dark:text-[#8b6b42]"
                    >
                      {h}
                    </th>
                  )
            ...
            <tbody>
              {lista.map((o, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-[#8b6b42]">
                    {o.responsable}
                  </td>
                  <td className="px-4 py-3 min-w-[100px]">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-gray-700 dark:text-[#c9a46b] w-8 text-right">
                        {o.progreso}%
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        estadoStyle[o.estado]
                      }`}
                    >
                      {estadoLabel[o.estado]}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-500 dark:text-[#8b6b42]">
              ...
              {lista.length === 0 && (
                <tr>
                  <td
                    colSpan={6}
                    className="px-4 py-8 text-center text-sm text-gray-400 dark:text-[#8b6b42]"
                  >
